...
plot.scrs <- plot.scrs %>%
  ...
  mutate(MonthL=fct_relevel(Month,"March", "May", "July")) 

plot.scrs

# vector 
vector.scrs <- scores(fit1, 
                      display="bp",  
                      scaling="species") %>% 
  as_tibble(rownames = "Plot") %>% 
  filter(Plot=="n_mow_events_befre_sampling") %>% 
  mutate(NMDS1=NMDS1*5.5, # rescale for better visualization
         NMDS2=NMDS2*5.5)
...
plot2 <- ggplot(data=plot.scrs, 
  ...
  geom_text_repel(data=sp.scrs, color="red4",
                  aes(x= NMDS1, y= NMDS2, label = Phenophase), 
